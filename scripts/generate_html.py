...
.table2-theme-label {
  ...
  color: var(--accent); padding: .6rem 0 .25rem; margin-top: .5rem;
  border-top: 1px solid var(--border2);
}
.table2-theme-label:first-child { border-top: none; margin-top: 0; padding-top: 0; }
...
.lang-sep { color: var(--border2); font-family: var(--font-mono); font-size: .65rem; }
...
let langMode     = 'both';    // 'both' | 'english' | 'tamil'
...
/* ── Frequency: Keyword × Year (per-theme when multiple) ────────── */
function renderTable2() {
  ...
  const years = state.years.size > 0 ? [...state.years].sort((a,b) => a-b) : ALL_YEARS;
  const themesToShow = [...state.themes].sort();
  const multiTheme = themesToShow.length > 1;
  ...
  for (const theme of themesToShow) {
    const base = QUESTIONS.filter(q =>
      (!state.paper || q.paper === state.paper) &&
      (!state.unit  || q.unit_number === state.unit) &&
      q.tags && q.tags.theme === theme
    );
    const kws = [...new Set(base.map(q => q.tags && q.tags.keyword).filter(Boolean))].sort();
    if (!kws.length) continue;

    if (multiTheme) {
      allHtml += '<div class="table2-theme-label">' + esc(theme) + '</div>';
    }
...
function renderCards(items, el) {
  ...
  for (const q of items) {
    ...
    html += '</div></div>';
    if ((langMode === 'both' || langMode === 'tamil')   && q.tamil)   html += '<div class="q-tamil">'   + esc(q.tamil)   + '</div>';
    if ((langMode === 'both' || langMode === 'english') && q.english) html += '<div class="q-english">' + esc(q.english) + '</div>';
    html += '</div>';
  }
  el.innerHTML = html;
}
  ...
  for (const paper of paperOrder) {
    ...
    for (const [theme, arr] of themeMap) {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        if ((langMode === 'both' || langMode === 'tamil')   && q.tamil)   html += '<div class="tamil-text">'   + esc(q.tamil)   + '</div>';
        if ((langMode === 'both' || langMode === 'english') && q.english) html += '<div class="english-text">' + esc(q.english) + '</div>';
        html += '</td>';
...
/* ── Boot ───────────────────────────────────────────────────────── */
// Default: latest year, Paper I, Unit I, first theme, all keywords
const _latestYear = ALL_YEARS[ALL_YEARS.length - 1];
state.years.add(_latestYear);
state.paper = 'Paper I';
state.unit  = 'I';

const _defBase = QUESTIONS.filter(q => q.paper === 'Paper I' && q.unit_number === 'I');
const _defThemes = [...new Set(_defBase.map(q => q.tags && q.tags.theme).filter(Boolean))].sort();
if (_defThemes.length) {
  state.themes.add(_defThemes[0]);
  const _defKws = [...new Set(
    _defBase
      .filter(q => q.tags && q.tags.theme === _defThemes[0])
      .map(q => q.tags && q.tags.keyword)
      .filter(Boolean)
  )];
  _defKws.forEach(k => state.keywords.add(k));
}
...
<div class="page-shell">
  <header>
    ...
    <div class="header-sub">Papers I&ndash;III &nbsp;&middot;&nbsp; 2019&ndash;2025 &nbsp;&middot;&nbsp; Tamil &amp; English &nbsp;&middot;&nbsp; Filter &amp; Study</div>
  ...
  <div class="filters-panel">
    ...
    <div class="filter-card" id="table1-card">
      <div class="filter-title-row">
        ...
        <button class="table-toggle-btn" id="table1-toggle-btn" onclick="toggleTable1()">Hide &#9650;</button>
      </div>
    ...
    <div class="filter-card" id="table2-card" style="display:none">
      <div class="filter-title-row">
        ...
        <button class="table-toggle-btn" id="table2-toggle-btn" onclick="toggleTable2()">Hide &#9650;</button>
      </div>
  ...
  <main>
    ...
    <div class="results-controls">
      <button class="inline-toggle-btn" id="view-toggle-btn" onclick="toggleView()">Card View</button>
      <span class="lang-sep">|</span>
      <button class="lang-btn active" data-lang="both"    onclick="setLang('both')">Both</button>
      <button class="lang-btn"        data-lang="english" onclick="setLang('english')">English</button>
      <button class="lang-btn"        data-lang="tamil"   onclick="setLang('tamil')">Tamil</button>
    </div>
    <div id="results"></div>
  </main>
</div>
