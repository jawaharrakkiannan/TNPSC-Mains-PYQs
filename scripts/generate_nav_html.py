...
Drill-down navigator HTML.
  Landing   : Paper × Unit table   (click unit → Unit page)
  Unit page : Theme × Subtheme/Keyword table  (click item → Detail page)
  Detail    : Keyword × Question × Year  (language selector: English/Tamil/Both)
"""
import json
import os
...
.page-nav {
  ...
  padding-bottom: .75rem; border-bottom: 1px solid var(--border); flex-wrap: wrap;
}
.back-btn {
...
.lang-btn {
  ...
  background: none; cursor: pointer; padding: .18rem .55rem; color: var(--text3);
  transition: border-color .1s, color .1s, background .1s;
}
.lang-btn.active { border-color: var(--accent); background: var(--accent2); color: var(--accent); font-weight: 500; }
.lang-btn:hover  { border-color: var(--accent); color: var(--accent); }
.lang-sep { color: var(--border2); font-family: var(--font-mono); font-size: .65rem; margin: 0 .1rem; }
...
.unit-num-cell {
  ...
  width: 3.2rem; min-width: 2.8rem;
  cursor: pointer; transition: background .1s;
}
.unit-name  { font-size: .85rem; color: var(--text); }
.unit-label { font-family: var(--font-mono); font-size: .58rem; color: var(--text3); margin-top: .15rem; }
.theme-cell {
...
const UNIT_ROMAN_ORDER = ['I', 'II', 'III'];

/* ── Build index ─────────────────────────────────────────────────── */
const paperUnits       = {};  // paper -> [unit_number]
const unitNames        = {};  // "paper\0unit" -> full unit name
const unitThemes       = {};  // "paper\0unit" -> [theme]
...
for (const q of QUESTIONS) {
  ...
  const uKey = p + '\x00' + u;
  if (!unitNames[uKey] && q.tags && q.tags.unit) unitNames[uKey] = q.tags.unit;

  if (!th) continue;

  if (!unitThemes[uKey]) unitThemes[uKey] = [];
  if (!unitThemes[uKey].includes(th)) unitThemes[uKey].push(th);

  const thKey = uKey + '\x00' + th;
  if (!themeSubthemes[thKey]) themeSubthemes[thKey] = [];
...
/* ── State ───────────────────────────────────────────────────────── */
let detailLang = 'english';
let _curPaper, _curUnit, _curTheme, _curSubtheme;

/* ── Helpers ─────────────────────────────────────────────────────── */
function esc(s) {
  return String(s ?? '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
}
function ja(v) { return esc(JSON.stringify(v)); }
...
function showPage(id) {
  ['pg-landing', 'pg-unit', 'pg-detail'].forEach(p => {
...
function renderLanding() {
  let html = '<div style="overflow-x:auto"><table class="nav-table"><thead><tr>'
           + '<th>Paper</th><th style="width:3.2rem;text-align:center">#</th><th>Unit Name</th>'
           + '</tr></thead><tbody>';

  for (const paper of PAPER_ORDER) {
    const units = (paperUnits[paper] || [])
      .slice().sort((a, b) => UNIT_ROMAN_ORDER.indexOf(a) - UNIT_ROMAN_ORDER.indexOf(b));
    const rowCount = units.length || 1;

    for (let i = 0; i < rowCount; i++) {
      html += '<tr>';
      if (i === 0) {
        html += '<td class="paper-cell" rowspan="' + rowCount + '">'
      ...
      if (units.length) {
        ...
        const clickAttr = ' onclick="showUnit(' + ja(paper) + ',' + ja(u) + ')"';
        html += '<td class="unit-num-cell clickable"' + clickAttr + '>' + esc(u) + '</td>'
              + '<td class="unit-cell clickable"' + clickAttr + '>'
              + '<div class="unit-name">' + esc(fullName) + '</div>'
              + '</td>';
      } else {
        html += '<td class="sub-cell muted" colspan="2">No units found</td>';
      }
      html += '</tr>';
    }
  }
...
/* ── Unit page: Theme × Subtheme / Keyword ───────────────────────── */
function showUnit(paper, unit) {
  _curPaper = paper;
  _curUnit  = unit;
  const fullName = unitNames[paper + '\x00' + unit] || ('Unit ' + unit);
  document.getElementById('unit-nav-title').textContent = paper + ' › ' + fullName;
  renderUnitPage(paper, unit);
  showPage('pg-unit');
}

function renderUnitPage(paper, unit) {
  const uKey   = paper + '\x00' + unit;
  const themes = unitThemes[uKey] || [];
  const el     = document.getElementById('unit-content');

  if (!themes.length) {
    el.innerHTML = '<p class="no-data">No themes tagged for this unit yet.</p>';
    return;
  }
  ...
  for (const theme of themes) {
    ...
    if (!items.length) {
      ...
      html += '<tr' + rowCls + '>'
            + '<td class="theme-cell clickable" rowspan="1"'
            + ' onclick="showDetail(' + ja(paper) + ',' + ja(unit) + ',' + ja(theme) + ',null)">'
            + esc(theme) + '</td>'
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const detailArgs = ja(paper) + ',' + ja(unit) + ',' + ja(theme) + ','
                       + (isKw ? 'null' : ja(item));
...
/* ── Detail page: Keyword × Question × Year ──────────────────────── */
function showDetail(paper, unit, theme, subtheme) {
  _curPaper    = paper;
  _curUnit     = unit;
  _curTheme    = theme;
  _curSubtheme = subtheme;
  const fullUnit = unitNames[paper + '\x00' + unit] || ('Unit ' + unit);
  const crumb    = paper + ' › ' + fullUnit + ' › ' + theme + (subtheme ? ' › ' + subtheme : '');
  document.getElementById('detail-nav-title').textContent = crumb;
  document.getElementById('detail-back-btn').onclick = () => showUnit(paper, unit);
...
function renderDetailQuestions() {
  const uKey  = _curPaper + '\x00' + _curUnit;
  const thKey = uKey + '\x00' + _curTheme;

  /* Top: subtheme × keywords summary */
  let topHtml = '';
  if (_curSubtheme) {
    const stKey = thKey + '\x00' + _curSubtheme;
    ...
    if (kws.length) {
      ...
      for (let i = 0; i < kws.length; i++) {
        topHtml += '<tr>';
        if (i === 0)
          topHtml += '<td class="theme-cell" rowspan="' + kws.length + '">' + esc(_curSubtheme) + '</td>';
    ...
    if (subs.length) {
      ...
      for (let i = 0; i < kws.length; i++) {
        topHtml += '<tr>';
        if (i === 0)
          topHtml += '<td class="theme-cell" rowspan="' + kws.length + '">' + esc(_curTheme) + '</td>';
        topHtml += '<td class="sub-cell">' + esc(kws[i]) + '</td></tr>';
      }
      topHtml += '</tbody></table></div>';
    }
  ...
  if (!kwOrder.length) {
    ...
    for (const kw of kwOrder) {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        if ((detailLang === 'both' || detailLang === 'tamil')   && q.tamil)
          qHtml += '<div class="q-tamil">'   + esc(q.tamil)   + '</div>';
        if ((detailLang === 'both' || detailLang === 'english') && q.english)
          qHtml += '<div class="q-english">' + esc(q.english) + '</div>';
...
<div class="page-shell">
  <header>
    ...
    <div class="header-sub">Papers I&ndash;III &nbsp;&middot;&nbsp; 2019&ndash;2025 &nbsp;&middot;&nbsp; Paper &rarr; Unit &rarr; Theme &rarr; Questions</div>
    <hr class="masthead-rule bottom">
  </header>

  <!-- Page 1: Landing (Paper × Unit) -->
  <div id="pg-landing">
    <div id="landing-content"></div>
  </div>

  <!-- Page 2: Unit (Theme × Subtheme/Keyword) -->
  <div id="pg-unit" style="display:none">
    <div class="page-nav">
      <button class="back-btn" onclick="showLanding()">&#8592; All Papers</button>
      <span class="page-nav-title" id="unit-nav-title"></span>
    </div>
    <div id="unit-content"></div>
  </div>

  <!-- Page 3: Detail (Keyword × Question × Year) -->
  <div id="pg-detail" style="display:none">
    ...
    <div class="lang-bar">
      <button class="lang-btn active" data-lang="english" onclick="setDetailLang('english')">English</button>
      <span class="lang-sep">|</span>
      <button class="lang-btn" data-lang="tamil"   onclick="setDetailLang('tamil')">Tamil</button>
      <span class="lang-sep">|</span>
      <button class="lang-btn" data-lang="both"    onclick="setDetailLang('both')">Both</button>
    </div>
